...
In total, there are `r nrow(baseline)` participants recruited in this study, and `r nrow(baseline_clean)` participants meet the inclusion criteria. 
...
amyloid_pivot = 
  pivot_longer(amyloid_clean, col = !id, names_to = "time", values_to = "amyloid β 42/40 ratio")
  
```

In total, there are `r nrow(amyloid_clean)` participants recruited in this study, and `r nrow(amyloid_clean)` participants meet the inclusion criteria. The variables evaluated are `r names (amyloid_clean)`.


Q1: What should be included in the write-up
...
combined_clean_full = 
  full_join(baseline_clean,amyloid_clean, by = "id") #participants without missing value in either dataset

combined_clean = 
  inner_join(baseline_clean,amyloid_clean, by = "id") #participants without missing value

combined_pivot = 
  inner_join(baseline_clean,amyloid_pivot, by = "id")

```

After cleaning the data, there are `r nrow(combined_clean_full)` participants who attended this study without missing value in either dataset. There are `r anti_join(amyloid_clean, baseline_clean, by = "id" ) |> nrow()` participants only appears in the amyloid dataset, and `r anti_join(baseline_clean, amyloid_clean, by = "id") |> nrow()` participants only appears in the baseline dataset.

The final dataset have `r nrow(combined_pivot)` rows and `r ncol(combined_pivot)` columns.The variables considered in this dataset are `r names(combined_pivot)`.
...
Q2: Cleaning: what data should be removed for baseline and amyloid?
baseline: having MCI at baseline (should i remove participants who never develop MCI?)
1. get mci after the experiment
2. or never get mci
the only group filter out is the people who's age of onset is younger than the current. 

amyloid: removed participatns have NA values (missing visit)
so we are not removing but consider change the arrangement of the data.
missing visit?

Q3: Do we need to find relationship between data etc? 

Q4: Check whether some participants appear in only the baseline or amyloid datasets, and comment on your findings.  
(do this with cleaned)
(just get the number or the exact id?) 

dimensions, variables that present, highlight interesting columns.  
